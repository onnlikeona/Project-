...
STARS_SHIFTS_IN_VARIANCE <- read_excel_allsheets("STARS_SHIFTS_IN_VARIANCE_RESULTS.xlsx")
...
a <- STARS_DETECTION_COUNTER(STARS_SHIFTS_IN_VARIANCE)
...
ggplot(PLOT_DATA, aes(x=CUTOFF_LENGTH, y=DETECTIONS, fill=PVALUE)) + 
  ...
  scale_y_continuous(name="Number of Detections",n.breaks = 10) + 
...
##############################
# 3.3 - Plotting probability of achieving 100% sensitivity.
##############################
...
CUTOFF_LENGTH <- c(rep("20" , 300) , rep("100" , 300) , rep("500" , 300))
PVALUE <- rep(c(rep("0.01",100) ,rep("0.05",100) ,rep("0.1",100)) , 3)
NUMBERS <- rbind(transpose(a$FalsePos[1,1:100]),transpose(a$FalsePos[2,1:100]),transpose(a$FalsePos[3,1:100]),
                 transpose(a$FalsePos[4,1:100]),transpose(a$FalsePos[5,1:100]),transpose(a$FalsePos[6,1:100]),
                 transpose(a$FalsePos[7,1:100]),transpose(a$FalsePos[8,1:100]),transpose(a$FalsePos[9,1:100]))
colnames(NUMBERS) <- "DETECTIONS"
PLOT_DATA <- data.frame(CUTOFF_LENGTH=as.factor(CUTOFF_LENGTH),'Probability Level'=as.factor(PVALUE),NUMBERS)
...
ggplot(PLOT_DATA, aes(x=CUTOFF_LENGTH, y=DETECTIONS, fill=PVALUE)) + 
  geom_boxplot() + 
  ggtitle("Frequency of False Positives across the Experiments") +
  theme_bw() +
  facet_wrap(~Probability.Level,labeller = "label_both") + 
  scale_y_continuous(name="Number of False Positives",n.breaks = 10) + 
  scale_x_discrete(limits=c("20","100","500"),name="Cutoff Length") +
  scale_fill_discrete(name = "Probability\nLevel",labels = c("0.01", "0.05", "0.10"))
...
ggplot(PLOT_DATA, aes(fill=PVALUE, y=PROPORTIONS, x=CUTOFF_LENGTH)) + 
  geom_bar(position="dodge", stat="identity") +
  ggtitle("Probability of STARS Making a False Detection") +
  theme_bw() +
  scale_x_discrete(limits=c("20","100","500"),name="Cutoff Length") +
